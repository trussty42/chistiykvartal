...
class SubmissionHistoryViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return SubmissionHistory.objects.filter(user=self.request.user).order_by('-created_at')
    ...
    def perform_create(self, serializer):
        ...
        if not has_organization_rights(self.request.user, point.organization):
            raise PermissionDenied('У вас нет прав на добавление истории для этого пункта')
        serializer.save()
